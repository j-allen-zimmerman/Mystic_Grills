...
public class WaiterMenu extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Label titleLabel = new Label("Waiter Menu");
        ...
        Button prepareButton = new Button("Serve Order");
    ...
    private void loadPreparedOrders() {
        ObservableList<Order> preparedOrders = FXCollections.observableArrayList(OrderController.getPreparedOrders());
        orderTable.setItems(preparedOrders);
    }

    private void prepareOrder(Order selectedOrder) {
        if (selectedOrder != null && "Prepared".equals(selectedOrder.getOrderStatus())) {
            OrderController.updateOrderStatus(selectedOrder.getOrderId(), "Served");
            loadPreparedOrders();
        } else {
            showAlert("Error", "No Prepared order selected or order already Served.");
        }
    }
